...
public class Handler implements Runnable {
    ...
    public void run() {
        try {
            ...
            //这里的异常处理是做了汇总，同样的，客户端也面临着正在与服务端进行写/读数据时，突然因为网络等原因，服务端直接断掉连接，这个时候客户端需要关闭自己并退出程序
        } catch (IOException e) {
    ...
    private void read() throws IOException {
        if (selectionKey.isValid()) {
            ...
            System.out.println(String.format("收到的消息: %s", new String(readBuffer.array(),0,readBuffer.array().length)));
