...
public class AsyncHandler implements Runnable {
    ...
    private static final ThreadFactory namedThreadFactory = new DefaultThreadFactory("demo-pool-%d");

    //Common Thread Pool
    private static final ExecutorService workers = new ThreadPoolExecutor(5, 200,
        ...
        if (selectionKey.isValid()) {
            try {
                ...
                if (count > 0) {
                    ...
                    selectionKey.cancel();
                    socketChannel.close();
                    System.out.println(String.format("序号为 %d SubChannel 触发：read时-------连接关闭", num));
                }
            } catch (IOException e) {
    ...
    private void readWorker() {
        try {
            System.out.println(String.format("序号为 %d SubChannel 触发：收到来自客户端 %s 的消息: %s",
                    num, socketChannel.getRemoteAddress(), new String(readBuffer.array())));
        } catch (IOException e) {
            e.printStackTrace();
            System.err.println("异步处理 read 业务时发生异常！异常信息：" + e.getMessage());
        }
        status = SEND;
        //注册写事件
    ...
    private void sendWorker() {
        try {
            sendBuffer.clear();
            sendBuffer.put(String.format("序号为 %d SubChannel 触发：我收到来自%s的信息：%s;",
            ...
            if (count < 0) {
                ...
                selectionKey.cancel();
                socketChannel.close();
                System.out.println(String.format("序号为 %d SubChannel 触发 ：send时-------连接关闭", num));
            }
